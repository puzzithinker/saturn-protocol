...
abstract contract BaseEthVault is ERC20, Ownable {
    ...
    function deposit(uint256 amount) external payable onlyEOA {
        ...
        IWETH(wantToken).deposit{value: amount}();
    ...
    function withdraw(uint256 amount) public onlyEOA {
        ...
        IWETH(wantToken).withdraw(amount.sub(withdrawFee));
        msg.sender.transfer(amount.sub(withdrawFee));
